...
void MotionGraphController::NextEdge(float initialTimeOffset)
{
    ...
    if(m_curEdge->blended)
    {
        ...
        m_clipControllers[0]->SetClip(m_curEdge->start->clip.RawPtr());
        m_clipControllers[0]->SetTime( m_curEdge->start->frame_num / m_curEdge->start->clip->GetClipFPS() );

        // and finish the blend at endclip/endframe
        m_clipControllers[1]->SetClip(m_curEdge->end->clip.RawPtr());
        m_clipControllers[1]->SetTime( m_curEdge->end->frame_num / m_curEdge->end->clip->GetClipFPS() - 
            m_curEdge->blendTime); 
    ...
    else // no blend needed, just playing a clip
    {
	    ...
	    float startFrame = (float)m_curEdge->start->frame_num;
    	float endFrame = (float)m_curEdge->end->frame_num - 1.f;
...
void MotionGraphController::SetRequestedPath( const MGPath& path )
{
    ...
    if(!m_algoGraph.Null() && m_algoGraph->GetNumEdges() > 0)
    {
		...
		Vec3 requestedDir = m_requestedPath.PointAtLength(1.0f) - m_requestedPath.Front();
        ...
        Vec3 animStart = GetAnimStart( m_curEdge->start->clip, m_curEdge->start->frame_num );
...
static const float kArcLengthSamplePeriod = 1/15.f;
										...
										SearchNode* parent)
{
    ...
    float startTime = edge->start->frame_num / edge->start->clip->GetClipFPS();
    float endTime = edge->end->frame_num / edge->end->clip->GetClipFPS();
	...
	if(edge->blended)
	{
        ...
        arcLength = EstimateBlendedArcLength(
            edge->start->clip.RawPtr(), startTime, 
            edge->end->clip.RawPtr(), endTime, edge->align_offset, edge->align_rotation,
            edge->blendTime);
	}
    else
    {
        edgeTime = endTime - startTime;
        arcLength = EstimateArcLength(edge->start->clip.RawPtr(), startTime, endTime);
    }
...
void MotionGraphController::AppendWalkEdges()
{
	...
	const int lastNumNeighbors = m_curEdge->end->outgoing.size();
	for(int i = 0; i < lastNumNeighbors; ++i) {
		AlgorithmMotionGraph::Edge* cur = m_curEdge->end->outgoing[i];
